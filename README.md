...
Dijkstra (on pg 84) provided the first 8 sequences satisfying the problem constraints. We used that as initial target to verify accuracy, though we did discover, as we mentioned above, a bug in our code that only surfaced at a sequence length of 87 digits (a bug which we repaired as discussed above). Our present code generates 100 adigit sequences in less than 8 ms running on a 2.5 GHz 4-core 64-bit Linux machine with 4 GB RAM. As far as reliability on longer sequences, we believe our code probably continues to be accurate, but as Dijkstra pointed out in his 1971 paper, 
